...
= Quarkus - Using the Mongo Client

Mongo is a well know NoSQL Database that is widely used.

In this guide, we see how you can get your REST services to use the Mongo database.
...
The solution is located in the `mongo-client` {quickstarts-tree-url}/mongo-client[directory]. TODO
...
mvn io.quarkus:quarkus-maven-plugin:{quarkus-version}:create \
    ...
    -Dpath="/fruits" \
    -Dextensions="resteasy-jsonb,mongo-client"
----

This command generates a Maven structure importing the RESTEasy/JAX-RS, JSON-B and Mongo Client extensions.

After this, the mongo extention has been added to your `pom.xml`. 
You can also add it manually if you don't want to use the maven quarkus plugin to create your project :
[source,xml]
----
    <dependency>
      <groupId>io.quarkus</groupId>
      <artifactId>quarkus-mongo-client</artifactId>
    </dependency>
----
...
Now create a `org.acme.rest.json.FuitService` that will be the business layer of our application and store/load the fruits from the mongo database.
...
The implementation is pretty straightforward and you just need to define your endpoints using the JAX-RS annotations and use the FuitService to list/add new fruits.

== Configuring the mongo database
The main property to configure is the URL to acces to MongoDB, almost all Mongo configuration can be put on the connection URI so we advise you to do so, you can find more information on the Mongo documentation site : https://docs.mongodb.com/manual/reference/connection-string/

A sample configuration should look like this:
[source,properties]
--
# configure mongo client for a replica set of two nodes
...
As by default, `MongoClient` is configure to access a local Mongo database on port 27017 (the default Mongo port), if you have a localy running database on this port, there is nothing more to do before being able to test it!
...
A reactive Mongo Client is included in Quarkus, it uses Vert.X reactive capabilities behind the scene. Using it is as easy as using the classic Mongo Client. You can rewrite the previous example to use it like the following.
...
== Simplifying Mongo Client usage
One of the drawback of using directly the Mongo Client, is that you need to manually transform your objects to/from a Mongo `Document`. To facilitate this there is two ways.

=== Using a Bson Codec
By using a Bson `Codec`, the Mongo Client will take care of the transformation of your domain object to/from a Mongo `Document` automatically.

First you need to create a Bson `Codec` that will tells Bson how to transform your entity to/from a Mongo `Document`. Here we use a `CollectibleCodec` as our object is retrievable from the database (it has a Mongo identifier), if not we would have used a `Codec` instead. More information on the codec documentation : https://mongodb.github.io/mongo-java-driver/3.10/bson/codecs.
...
Then you need to create a `CodecProvider` to linked this `Codec` to the `Fruit` class.
...
Quarkus will take care of registrating the `Codec` for you.

Finally, when getting the `MongoCollection` from the databalse you can use directly the `Fuit` class instead of the `Document` one, the codec will automatically map the `Document` to/from your `Fruit` class.
...
=== Using a JSON parser
You can use a JSON parser to parse your object to/from JSON and create or read a Mongo `Document` from a JSON.

Here is an example of creating a Mongo `Document` from your Object using a JSON parser (Gson here, but other parser can be used).
...
And here is an example of creating your object from a Mongo `Document` using a same JSON parser.
...
Fruit entity = gson.fromJson(json, Fruit.class;
----

== Building a native executable
To run Mongo in native mode with authentification, you need to add specific configurations to the `quarkus-maven-plugin` that is defined in the `native` profile :

[source,xml]
----
<configuration>
    <enableHttpUrlHandler>true</enableHttpUrlHandler>
    <!-- The following two lines are mandatory when using Mongo Authentification -->
...
[WARNING]
====
Currently, Quarkus doesn't support `mongodb+srv` protocol on the MoongoDB connection string.
====

== Conclusion

Accessing a Mongo database from a Mongo Client is easy with Quarkus as it provides configuration and native support for it.
...
Minumum number of connections in the connection pool.

`quarkus.mongodb.max-connection-idle-time`::
Maximum idle time in milliseconds of a pooled connection. A connection that exceeds this limit will be closed.

`quarkus.mongodb.max-connection-life-time`::
Maximum life time in milliseconds of a pooled connection. A connection that exceeds this limit will be closed.

`quarkus.mongodb.wait-queue-timeout`::
The maximum wait time in milliseconds that a thread may wait for a connection to become available.

`quarkus.mongodb.maintenance-frequency`::
Configures the time period in milliseconds between runs of the maintenance job. Default is 0.

`quarkus.mongodb.maintenance-initial-delay`::
Configures period of time in milliseconds to wait before running the first maintenance job on the connection pool.
...
How long in milliseconds a connection can take to be opened before timing out.

`quarkus.mongodb.socket-timeout`::
How long in milliseconds a send or receive on a socket can take before timing out.
...
How long in milliseconds the driver will wait for server selection to succeed before throwing an exception.

`quarkus.mongodb.local-threshold`::
When choosing among multiple MongoDB servers to send a request, the driver will only send that request to a server whose ping time in milliseconds is less than or equal to the server with the fastest ping time plus the local threshold.

`quarkus.mongodb.heartbeat-frequency`::
The frequency in milliseconds that the driver will attempt to determine the current state of each server in the cluster.
...
=== Write concern
`quarkus.mongodb.write-woncern.safe`::
...
`quarkus.mongodb.write-woncern.journal`::
...
`quarkus.mongodb.write-woncern.w`::
...
`quarkus.mongodb.write-woncern.retry-writes`::
If set to `true`, the driver will retry supported write operations if they fail due to a network error. The default is `false`.

`quarkus.mongodb.write-woncern.w-timeout`::
When set, the driver adds `wtimeout : ms`  to all write commands. It requires `safe` to be `true`. Time given in ms.
